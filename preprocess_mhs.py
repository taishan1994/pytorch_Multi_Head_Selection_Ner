...
class BertFeature(BaseFeature):
    def __init__(self, token_ids, attention_masks, token_type_ids, pair_labels=None, entity_labels=None):
        super(BertFeature, self).__init__(
            token_ids=token_ids,
            attention_masks=attention_masks,
            token_type_ids=token_type_ids)
        # labels
        self.pair_labels = pair_labels  # token对标签
        self.entity_labels = entity_labels  # 句子是否有该实体标签
...
def convert_bert_example(ex_idx, example: InputExample, tokenizer: BertTokenizer,
    ...
    label2id = {v:i for i,v in enumerate(labels, 1)}  # 从1开始，预留0位置
    ...
    pair_labels_label_ids = None
    entity_label_ids = None

    # information for dev callback
    # ========================
    pair_label_ids = np.zeros((len(raw_text), len(raw_text)))
    entity_label_ids = [0] * len(labels)
    ...
    for ent in entities:
        ...
        pair_label_ids[ent_start][ent_end] = label2id[ent_type]
        entity_label_ids[label2id[ent_type]-1] = 1


    if len(pair_label_ids) > max_seq_len - 2:
        pair_label_ids = pair_label_ids[:max_seq_len - 2, :max_seq_len - 2]

    pair_label_ids = np.pad(pair_label_ids, ((1,1),(1,1)), 'constant', constant_values=(0,0)) 
    # print(raw_text)
    # print(entities)
    # print(pair_label_ids)

    # pad
    if len(pair_label_ids) < max_seq_len:
        pad_length = max_seq_len - len(pair_label_ids)
        pair_label_ids = np.pad(pair_label_ids, ((0, pad_length),(0, pad_length)), "constant", constant_values=(0,0))  # CLS SEP PAD label都为O
        # print(len(pair_label_ids), len(pair_label_ids[0]))

    assert len(pair_label_ids) == max_seq_len
    # ========================
    ...
    if ex_idx < 3:
        ...
        print(tokenizer.decode(token_ids[:len(raw_text)+1]))
        logger.info(f'text: {" ".join(tokens)}')
        logger.info(f"token_ids: {token_ids}")
        logger.info(f"attention_masks: {attention_masks}")
        logger.info(f"token_type_ids: {token_type_ids}")
        logger.info(f"pair_label_ids: {pair_label_ids}")
        logger.info(f"entity_label_ids: {entity_label_ids}")
...
def save_file(filename, data, id2ent, id2label):
    ...
    for feature,tmp_callback in zip(features, callback_info):
        ...
        for word, bio_label, att_label in zip(text, feature.bio_labels[1:len(text)+1], feature.att_labels[1:len(text)+1]):
...
if __name__ == '__main__':
    ...
    if dataset == "c":
        ...
        train_data = get_data(processor, mid_data_path, "train.json", "train", None, labels, args)
        # save_file(os.path.join(mid_data_path,"cner_{}_cut.txt".format(args.max_seq_len)), train_data, id2ent, id2label)
        dev_data = get_data(processor, mid_data_path, "dev.json", "dev", None, labels, args)
        test_data = get_data(processor, mid_data_path, "test.json", "test", None, labels, args)
